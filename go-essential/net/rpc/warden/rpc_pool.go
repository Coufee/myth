...
func newPool(size int, ttl time.Duration, idle int, ms int) *RpcPool {
	if ms <= 0 {
		ms = 1
	}
	if idle < 0 {
...
func (p *RpcPool) getConn(ctx context.Context, addr string, opts ...grpc.DialOption) (*poolConn, error) {
	now := time.Now().Unix()
	p.Lock()
	sp, ok := p.conns[addr]
	if !ok {
...
func (p *RpcPool) release(addr string, conn *poolConn, err error) {
	p.Lock()
