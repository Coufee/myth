...
func newPatternRender(format string, handlerType string) Render {
	p := &pattern{
...
func (p *pattern) Render(w io.Writer, lv LogLevel, d map[string]interface{}) (err error) {
	...
	if p.handlerType == handlerTypeStdOut && (_Os == osWindow || _Os == osMac) {
		_, err = w.Write([]byte(levelColor[lv].Add(builder.String())))
	} else {
...
func colorData(map[string]interface{}) string {
	return ""
}
